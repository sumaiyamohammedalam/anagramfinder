#include <stdio.h>
#include <string.h>

int main() {
    // --- Task 2: Create counters --
    int counter1[4] = {0, 0, 0, 0};
    int counter2[4] = {0, 0, 0, 0};

    // --- Task 3: Strings to compare ---
    char s1[] = "dbb cccccaacb cdbababdcdcdab dcdad";
    char s2[] = "bbbcc bdddccccad cdbbaaacaccdabdd";

    // --- Task 4 & 5: Count characters in s1 --
    for (int i = 0; i < strlen(s1); i++) {
        if (s1[i] == 'a') counter1[0]++;
        else if (s1[i] == 'b') counter1[1]++;
        else if (s1[i] == 'c') counter1[2]++;
        else if (s1[i] == 'd') counter1[3]++;
        // ignore spaces
    }

    // --- Task 6: Count characters in s2 ---
    for (int i = 0; i < strlen(s2); i++) {
        if (s2[i] == 'a') counter2[0]++;
        else if (s2[i] == 'b') counter2[1]++;
        else if (s2[i] == 'c') counter2[2]++;
        else if (s2[i] == 'd') counter2[3]++;
    }

    // --- Task 7: Flag for mismatch ---
    int flag = 0;

    // --- Task 8 & 9: Compare counters ---
    for (int i = 0; i < 4; i++) {
        if (counter1[i] != counter2[i]) {
            flag = 1;
            break; // no need to check further
        }
    }

    // --- Task 10: Print result ---
    if (flag == 0)
        printf("Anagram!\n");
    else
        printf("Not Anagram!\n");

    return 0;
}
